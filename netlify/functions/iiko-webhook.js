...
async function getIikoToken() {
    ...
    try {
        // ИСПРАВЛЕННЫЙ URL
        const { data } = await axios.post(`${iikoApiUrl}/api/1/access_token`, {
            apiLogin: iikoApiKey
        });
            ...
            for (const category of menuData.itemCategories) {
                if (category.items) {
                    for (const item of category.items) {
                        ...
                        } else if (item.buttonImageUrl) {
                            imageSource = item.buttonImageUrl;
                        } else if (item.itemSizes?.[0]?.buttonImageUrl) {
                            imageSource = item.itemSizes[0].buttonImageUrl;
                        }
                        
                        let imageIdToSave = null;
                        if (imageSource) {
                            if (imageSource.startsWith('http') || imageSource.startsWith('https')) {
                                imageIdToSave = imageSource;
                            } else if (imageSource.includes('imageId=')) {
                                const imageIdPart = imageSource.split('imageId=')[1];
                                if (imageIdPart) {
                                    imageIdToSave = imageIdPart.split('.')[0];
                                }
                            } else {
                                imageIdToSave = imageSource;
                            }
